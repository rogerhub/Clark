...
class SettingsController < ApplicationController
  ...
  def uploadpicture
    ...
    `convert "#{Rails.root.join(params[:profilepic].tempfile.path)}" -resize 256x256^ -gravity center -extent 256x256 "#{finalpath}"`
  ...
  def changepassword
    return render :text => "Wrong old password inputted." if $user.password == hash_password(params[:cpwold])
    $user.password = hash_password(params[:cpwnew])
